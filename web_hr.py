...
def add_employee():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute('''
                INSERT INTO employees (name, username, password, department, role, photo, hourly_rate)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (name, username, password, dept, role, filename, hourly_rate))
            conn.commit()
        except Exception as e:
            print(f"خطأ: {e}")
...
def check_out():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM attendance WHERE user_id=? AND date=? AND check_out_time IS NULL", (user_id, today))
        record = cursor.fetchone()
        if record:
            cursor.execute("UPDATE attendance SET check_out_time=? WHERE user_id=? AND date=?",
...
def set_zone():
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("UPDATE settings SET office_lat=?, office_lng=?, allowed_radius=? WHERE id=1",
...
def save_salary():
    ...
    try:
        cursor.execute('''
            INSERT INTO payroll_history (user_id, month, total_hours, hourly_rate, basic_salary, bonus, deductions, net_salary)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def add_announcement():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO announcements (title, content, created_at) VALUES (?, ?, ?)",
